# 2018/08/07 D_ENTRP_YYYYMMDD.csvファイルからファイル情報を取得し、ディレクトリ構成を再構築するバッチ
# 実行方法：
#           ①本スクリプトの直下にCSVサブフォルダを置く
#           ②$CONFIG_IMAGESOURCE_PATHに取り込み用イメージファイルのパスを設置(例：C:\Test\ImgForder)
#
...
function remakingDirectory($csvFileFullName)
{
    ...
    try{
        ...
        foreach ($CsvData in $CsvDataArray)
        {
            ...
            $ImageFileNameFullPath = $CONFIG_IMAGESOURCE_PATH
